...
def get_groq_client():
    """Initialize and return Groq client using API key from Streamlit secrets or .env."""
    ...
    try:
        ...
        print(f"⚠️ Could not read from st.secrets: {e}")
    
    # If not found, try .env file (local)
    if not api_key:
        load_dotenv()
